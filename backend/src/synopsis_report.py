"""Turn synopsis artifacts into a narrative markdown report."""
...
def _latest_synopsis_run(output_dir: Path) -> Path:
    """Return the newest run directory that contains ``synopsis_metrics.json``."""
...
def generate_synopsis_report(
    ...
    """Read a synopsis run's metrics/correlations and write a dated markdown report.

    Uses ``run_dir`` when given, else the latest synopsis run under ``output_dir``.
    Returns the path of the report written into ``reports_dir``.
    """
